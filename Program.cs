...
try
{
    ConvertSvgToWebp(options.InputPath, options.Output1Path, options.Width1, options.Height1, options.Quality1);
    ConvertSvgToWebp(options.InputPath, options.Output2Path, options.Width2, options.Height2, options.Quality2);
...
static void ConvertSvgToWebp(string inputSvg, string outputWebp, int width, int height, int quality)
{
    if (width <= 0 || height <= 0)
    {
...
static Options ParseArgs(string[] args)
{
    ...
    for (var i = 0; i < args.Length; i++)
    {
        ...
        {
            throw new ArgumentException($"Valore mancante per parametro {current}");
        }
    ...
    return new Options(input, output1, output2, width1, height1, width2, height2, quality1, quality2);
}
